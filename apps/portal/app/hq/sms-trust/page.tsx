...
async function pauseAutomationAction(formData: FormData) {
  ...
  if (!orgId) {
    redirect("/hq/sms-trust?error=missing-org");
  }

  await requireInternalUser("/hq/sms-trust");
  await pauseOrgSmsAutomation(orgId);

  revalidatePath("/hq/sms-trust");
  revalidatePath("/app/settings");
  revalidatePath("/app");

  redirect(`/hq/sms-trust?paused=${encodeURIComponent(orgId)}`);
}
