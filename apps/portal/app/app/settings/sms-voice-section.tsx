...
function getToneOptions(locale: string): ToneOption[] {
  if (locale.startsWith("es")) {
    return [
      {
        tone: "FRIENDLY",
        label: "Amigable y casual",
        description: "Calido, conversacional y sin presion.",
      },
      {
        tone: "PROFESSIONAL",
        label: "Profesional y pulido",
        description: "Tono de oficina claro y seguro.",
      },
      {
        tone: "DIRECT",
        label: "Directo al punto",
        description: "Corto, eficiente y rapido de responder.",
      },
      {
        tone: "SALES",
        label: "Ventas de alta energia",
        description: "Urgencia con impulso hacia la reserva.",
      },
      {
        tone: "PREMIUM",
        label: "Premium / lujo",
        description: "Tono elevado con fuerte sensacion de confianza.",
      },
      {
        tone: "BILINGUAL",
        label: "Bilingue amigable",
        description: "Ingles y espanol en un mismo flujo.",
      },
      {
        tone: "CUSTOM",
        label: "Personalizado",
        description: "Usa tu propio texto con protecciones.",
      },
    ];
  }
...
function getSmsVoiceCopy(locale: string): SmsVoiceCopy {
  if (locale.startsWith("es")) {
    return {
      ...
      businessHoursBody:
        "Pon la ventana real de estimados y el agente SMS priorizara esos horarios primero.",
      ...
      agentPlaybookBody:
        "Haz que el dueño responda esto como habla en la vida real. Mientras mas claro sea, mejor calificara y agendara la IA.",
      primaryGoal: "1. ¿Cuál es la meta principal del agente SMS?",
      businessContext:
        "2. ¿Cómo maneja este negocio los nuevos leads de estimados?",
      ...
      handoffTriggers:
        "6. ¿Cuándo debe dejar de automatizar y pasar a un humano?",
      ...
      useInboundPhoneAsCallback:
        "11. ¿Debe usar el número del SMS como callback por defecto?",
      customTemplates: "Plantillas personalizadas",
      customTemplatesBody:
        "Manten esto humano y natural. Evita palabras como bot/AI/automated. La logica STOP sigue bloqueada.",
  ...
  return {
    ...
    businessHoursBody:
      "Set the real estimate window and the SMS agent will prioritize those times first.",
    ...
    agentPlaybookBody:
      "Have the owner answer these in plain language. The clearer they are, the more human and accurate the AI will be.",
    primaryGoal: "1. What is the main job you want the SMS agent to do?",
    businessContext:
      "2. How does this business normally handle new estimate leads?",
    servicesSummary: "3. What services should it actively take by SMS?",
    serviceAreaSummary:
      "4. What cities or service areas do you actually serve?",
    requiredDetails: "5. What details must it collect before scheduling?",
    handoffTriggers:
      "6. When should it stop automating and hand off to a human?",
    ...
    useInboundPhoneAsCallback:
      "11. Should it use the inbound SMS number as the default callback number?",
...
function getSmsAgentPlaybookExamples(locale: string): SmsAgentPlaybookExamples {
  if (locale.startsWith("es")) {
    return {
      primaryGoal:
        "Agendar estimados en sitio para Cesar y dejar leads bien calificados en el CRM.",
      businessContext:
        "Somos Velocity Landscapes. Queremos responder rapido, sacar tipo de trabajo y dirección, y agendar si el lead encaja.",
      servicesSummary:
        "Muros de contención, hardscape, limpieza de yarda, drenaje, pavers y trabajos de landscape que sí queremos cotizar.",
      serviceAreaSummary:
        "Tacoma, Puyallup, Lakewood, University Place y zonas cercanas donde sí damos servicio.",
      requiredDetails:
        "Tipo de trabajo, dirección completa, ciudad, acceso/gate code si aplica, y plazo del cliente.",
      handoffTriggers:
        "Pásalo a humano si el cliente quiere llamar, pide precio exacto por texto, tiene un proyecto raro, o quiere un horario fuera de nuestra ventana.",
      toneNotes:
        "Humano, seguro, servicial y nada robótico. Como recepcionista de oficina, no vendedor agresivo.",
      estimatorName: "Cesar",
      schedulingNotes:
        "Cesar prefiere hacer estimados más tarde, normalmente entre 4pm y 7pm. Si no puede ese horario, pásalo a humano.",
      doNotPromise:
        "No prometas precio final, fecha de inicio garantizada, ni que podemos hacer cualquier trabajo sin que Cesar lo vea.",
    };
  }

  return {
    primaryGoal:
      "Book on-site estimates for Cesar and leave clean, qualified lead details in the CRM.",
    businessContext:
      "We are Velocity Landscapes. We want fast replies, collect the job type and address, and book the estimate if the lead is a fit.",
    servicesSummary:
      "Retaining walls, hardscape, yard cleanup, drainage, pavers, and landscaping work we actively want to quote.",
    serviceAreaSummary:
      "Tacoma, Puyallup, Lakewood, University Place, and nearby areas we actually serve.",
    requiredDetails:
      "Work needed, full address, city, gate/access notes if relevant, and the customer's timeframe.",
    handoffTriggers:
      "Hand off if the customer wants a call, asks for exact pricing over text, has an unusual project, or wants times outside our preferred estimate window.",
    toneNotes:
      "Human, calm, helpful, and confident. It should sound like a real office coordinator, not a bot or aggressive salesperson.",
    estimatorName: "Cesar",
    schedulingNotes:
      "Cesar prefers to run estimates later in the day, usually between 4pm and 7pm. If that window does not work, hand it to a human.",
    doNotPromise:
      "Do not promise final pricing, guaranteed start dates, or that we take the job before Cesar reviews it.",
  };
}
...
type SmsVoiceSectionProps = {
  ...
  canManage: boolean;
  canManageAutomationControls: boolean;
  automationLockedNotice: string | null;
  initialTone: SmsTone;
...
function getPreviewPack(input: {
  ...
  return {
    ...
    askTimeframe:
      input.customTemplates.askTimeframe.trim() || base.askTimeframe,
    offerBooking:
      input.customTemplates.offerBooking.trim() || base.offerBooking,
    bookingConfirmation:
      input.customTemplates.bookingConfirmation.trim() ||
      base.bookingConfirmation,
...
export function SmsVoiceSection(props: SmsVoiceSectionProps) {
  ...
  const [customTemplates, setCustomTemplates] =
    useState<SmsVoiceCustomTemplates>(props.initialCustomTemplates);
  ...
  const slotList =
    props.previewSlots.length > 0
      ? props.previewSlots.join("  ")
      : "A) Tomorrow 10:00am  B) Thu 2:00pm  C) Fri 9:00am";
  const [
    slot1 = "A) Tomorrow 10:00am",
    slot2 = "B) Thu 2:00pm",
    slot3 = "C) Fri 9:00am",
  ] =
    props.previewSlots.length > 0
      ...
      <section className="sms-voice-controls">
        ...
        <div className="sms-voice-card-grid">
          {toneOptions.map((option) => {
            const tonePack = getSmsToneTemplates({
              tone: option.tone,
              locale: props.locale,
            });
            return (
              <label
                key={option.tone}
                className={`sms-voice-card ${tone === option.tone ? "active" : ""}`}
              >
                <input
                  type="radio"
                  name="smsTone"
        ...
        {props.automationLockedNotice ? (
          <p className="muted">{props.automationLockedNotice}</p>
        ) : null}
        <label className="inline-toggle">
          <input
            type="checkbox"
            name="autoReplyEnabled"
            defaultChecked={props.initialAutoReplyEnabled}
            disabled={!props.canManageAutomationControls}
          />
        ...
        <label className="inline-toggle">
          <input
            ...
            defaultChecked={props.initialFollowUpsEnabled}
            disabled={!props.canManageAutomationControls}
          />
        ...
        <label className="inline-toggle">
          <input
            ...
            defaultChecked={props.initialAutoBookingEnabled}
            disabled={!props.canManageAutomationControls}
          />
        ...
        <div className="sms-voice-grid-two">
          <label>
            ...
            <input
              ...
              defaultValue={props.initialWorkingHoursStart}
              disabled={!props.canManage}
            />
          </label>
          <label>
            ...
            <input
              ...
              defaultValue={props.initialWorkingHoursEnd}
              disabled={!props.canManage}
            />
          </label>
          <label>
            ...
            <input
              ...
              defaultValue={props.initialSlotDurationMinutes}
              disabled={!props.canManage}
            />
          </label>
          <label>
            ...
            <input
              ...
              max={120}
              name="bufferMinutes"
              defaultValue={props.initialBufferMinutes}
              disabled={!props.canManage}
            />
          </label>
          <label>
            ...
            <input
              ...
              max={14}
              name="daysAhead"
              defaultValue={props.initialDaysAhead}
              disabled={!props.canManage}
            />
          </label>
          <label>
        ...
        <div className="sms-voice-custom-editor">
          ...
          <label className="inline-toggle">
            <input
              ...
              defaultChecked={
                props.initialAgentPlaybook.useInboundPhoneAsCallback
              }
              disabled={!props.canManage}
            />
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) =>
                  setCustomTemplates((prev) => ({
                    ...prev,
                    askTimeframe: event.target.value,
                  }))
                }
                disabled={!props.canManage}
              />
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) =>
                  setCustomTemplates((prev) => ({
                    ...prev,
                    offerBooking: event.target.value,
                  }))
                }
                disabled={!props.canManage}
              />
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) =>
                  setCustomTemplates((prev) => ({
                    ...prev,
                    bookingConfirmation: event.target.value,
                  }))
                }
                disabled={!props.canManage}
              />
